...
namespace Bolt\Installer\Manager;
...
class ComposerManager
{
    ...
    public function updateProjectConfig(array $newConfig)
    {
        ...
        $projectConfig = array_filter($projectConfig, function ($value) {
    ...
    private function getProjectConfig()
    {
        ...
        if (!is_writable($composerJsonPath) && is_writable($composerJsonPath . '.dist')) {
            return json_decode(file_get_contents($composerJsonPath . '.dist'), true);
        }
    ...
    private function initComposerLockFile()
    {
        $composerLockPath = $this->projectDir . '/composer.lock';
        if (!is_writable($composerLockPath) && is_writable($composerLockPath . '.dist')) {
            $this->fs->copy($composerLockPath . '.dist', $composerLockPath);
        }
    }
